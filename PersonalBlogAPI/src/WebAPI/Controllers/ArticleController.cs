...
public class ArticleController : BaseController
{
    ...
    {
        BaseResponse<ArticleWithTagsDto> result = await Mediator.Send(new GetArticleByIdIncludeTagsQuery(id), cancellationToken);
        return Ok(result);
    }
    ...
    {
        BaseResponse<IReadOnlyCollection<ArticleDto>> result = await Mediator.Send(new GetAllArticlesQuery(), cancellationToken);
        return Ok(result);
    }
    ...
    {
        BaseResponse<ArticleWithTagsDto> result = await Mediator.Send(command, cancellationToken);
